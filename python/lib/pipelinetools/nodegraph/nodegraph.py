...
class NodeNetwork(object):
    ...
    def connect(self, src_param, dst_param, passthru=False):
        ...
        if not passthru:
            if src_param.node.parent_network != dst_param.node.parent_network:
                raise Exception("Unable to connect '%s' to '%s'. Cannot connect parameters accross "\
                    "network." % (src_param, dst_param))
    ...
    def collapse(self, nodes=None):
        ...
        subnet_node = self.add_node("subnet", "SubnetNode")

        sunet_input_node = subgraph_node.child_node("input")
        subnet_output_node = subgraph_node.child_node("output")
