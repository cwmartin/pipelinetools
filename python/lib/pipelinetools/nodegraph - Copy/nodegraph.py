...
class NodeGraph(object):
    ...
    def nodes(self):
        ...
        """
        return nx.topological_sort(self._graph, nbunch=self._graph.nodes())        
    ...
    def add_node(self, name, ntype):
        ...
        """        
        print "---->ADDING NODE", self._parent_node, name, ntype
        node_type = registry.NodeRegistry.get(ntype)        
        node_name = self._get_unique_node_name(name)
        node = node_type(node_name, parent=self)
    ...
    def collapse(self, nodes=None):
        ...
        #input_node = subgraph_node.nodegraph.node("input")
        #output_node = subgraph_node.nodegraph.node("output")
        ...
        if subgraph:
            ...
            for node in nodes:
                ...
                print "move", subgraph_node.nodegraph, "to", node._parent

                node._parent.move_node(node, subgraph_node.nodegraph)
